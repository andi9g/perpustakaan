<div class="card-header">
    <div class="row">
        <div class="col">
            <h4 style="text-transform: capitalize;font-weight: bold;"><i class="fa fa-users"></i>&nbsp;&nbsp;<?php echo str_replace('_',' ',$_GET['pages'])?></h4>
        </div>
        <div class="col text-right">
            <a href="?pages=<?php echo $_GET['pages']?>&aksi=tambah_siswa" class="btn <?php if($_SESSION['sidebarku']=='light'){?>btn-success<?php }else {echo 'btn-danger';}?> btn-sm">
                <i class="fa fa-plus"> </i> Tambah Siswa
            </a>
        </div>
    </div>
    
</div>


<div class="card-body">
    <table id="example1" class="table <?php if($_SESSION['sidebarku']=='light'){?>table-bordered <?php };?>table-striped tableku22" style="text-transform: capitalize;">
        <thead>
        <tr>
            <th>Nis</th>
            <th>Nama Siswa</th>
            <th>No Hp</th>
            <th>Alamat</th>
            <th hidden>Reset Password</th>
            <th>Aksi</th>
        </tr>
        </thead>
        <tbody>
            <?php
                foreach($db->tampil_siswa() as $tampil){
            ?>
            <tr>
                <td style="font-weight: bold;"><?php echo $tampil['nis']?></td>
                <td><?php echo $tampil['nama']?></td>
                <td><?php echo $tampil['hp']?></td>
                <td><?php echo $tampil['alamat']?></td>
                <td hidden><?php echo $tampil['password']?></td>
                <td>
                    <form method="post" class="d-inline mr-1 p-0">
                        <button onclick="return confirm('Yakin ingin mehapus?')" type="submit" class="btn btn-warning btn-xs px-2 p-0" name="hapus_siswa" value="<?php echo $tampil['nis'];?>"><i class="fa fa-trash text-dark"></i></button>
                    </form>
                    <a href="index.php?pages=<?php echo $_GET['pages'];?>&aksi=ubah_siswa&id=<?php echo $tampil['nis'];?>" class="btn btn-info btn-xs px-2 p-0" "><i class="fa fa-pencil text-dark"></i></a>
                </td>
            </tr>
            
        <?php }?>
       
        </tbody>
    </table>
</div>



<?php
    if(isset($_POST['hapus_siswa'])){
        $ket = $db->hapus_siswa($_POST['hapus_siswa']);
        
    }
?>

